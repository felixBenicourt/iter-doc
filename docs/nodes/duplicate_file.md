...
The **Duplicate File** node is used to duplicate a file from one location to another. It requires two inputs:

- **Input Top**: The path of the original file that you want to duplicate.
- **Input Bottom**: The path where you want the new duplicated file to be saved.

This node enables file management operations directly within the node graph, making it easier to automate tasks related to file duplication.
...
- **Input Top**:
  - **path**: The path of the original file to be duplicated.
  
- **Input Bottom**:
  - **path**: The destination path where the duplicated file will be saved.
