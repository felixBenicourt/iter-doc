...
The **Join Path** node combines two attributes into a single path. It takes the two input attributes and merges them into one unified path. This node is useful when you need to combine multiple attributes from different fluxes into a single, cohesive path for further processing.

---

### Parameters

- **Attribute top**: Name of the first attribute path to be joined.
- **Attribute bottom**: Name of the second attribute path to be joined.
- **New name attribute**: The name for the newly created attribute path that results from the join.
